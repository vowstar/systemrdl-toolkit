...
Inja provides many built-in filters:
...
The tool uses Inja's built-in functions - no custom callbacks needed:

```jinja2
{{ node.inst_name | upper }}                  <!-- Built-in upper() -->
{{ node.inst_name | lower }}                  <!-- Built-in lower() -->
{{ field.inst_name | replace("[", "_") | replace("]", "_") }}  <!-- Built-in replace() -->
```

#### Template Naming Convention

Follow this naming pattern for template files:

- `test_j2_<purpose>.<ext>.j2` - Template files
- Place in `test/` directory
- Use descriptive purpose names: `header.h`, `doc.md`, `verilog.v`, etc.

### Testing Template Rendering

Use the provided validation script to verify template rendering:

```bash
# Run all template rendering tests
python3 script/template_render_validator.py

# Manual testing
./build/systemrdl_render test/test_basic_chip.rdl -t test/test_j2_header.h.j2 -v
```

### Output File Naming

If no output file is specified (`-o`), the tool auto-generates names:

| Template Pattern | Generated Name |
|------------------|----------------|
| `test_j2_header.h.j2` | `design_header.h` |
| `test_j2_doc.md.j2` | `design_doc.md` |
| `test_j2_verilog.v.j2` | `design_verilog.v` |
| `custom_template.j2` | `design_rendered.txt` |
...
```bash
Error: Cannot open template file template.j2
```
...
```bash
Template rendering error: [inja.exception.render_error] variable 'missing_var' not found
```

→ Check variable names in template match provided data structure

**Missing properties:**

```jinja2
{% if field.properties.reset %}
  Reset: {{ field.properties.reset }}
{% else %}
  Reset: Not specified
{% endif %}
```
...
```bash
# Generate multiple outputs from one RDL file
./systemrdl_render design.rdl -t test/test_j2_header.h.j2 -o design.h
./systemrdl_render design.rdl -t test/test_j2_doc.md.j2 -o design_docs.md
./systemrdl_render design.rdl -t test/test_j2_verilog.v.j2 -o design_rtl.v
```
...
for rdl in designs/*.rdl; do
    base=$(basename "$rdl" .rdl)
    ./systemrdl_render "$rdl" -t test/test_j2_header.h.j2 -o "generated/${base}.h"
done
```
